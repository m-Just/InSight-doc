...
IDLE_STABLE_SECONDS="${IDLE_STABLE_SECONDS:-180}"
...
gpu_has_memory_allocations() {
  ...
  if ! query="$(nvidia-smi --query-gpu=index,memory.used --format=csv,noheader,nounits 2>/dev/null)"; then
    log "nvidia-smi query failed for GPUs ${gpus}; treating them as busy"
    return 0
  fi
...
wait_for_idle_gpus() {
  ...
  while true; do
    if gpu_has_memory_allocations "$gpus"; then
      idle_started=""
...
run_eval_one() {
  ...
  local scale_value="$5"
  local agent_cfg="$6"
  local model_label="base_no_tool_no_system_fast_hf_val_only"
...
run_lane() {
  ...
  wait_for_idle_gpus "$gpus"
  run_eval_one "$lane" "$repeat_id" "$gpus" 07 0.7 recipe/vsearch/config/agent_insight_qwen_agent_zoom_factor2_area3500_rescale07.yaml
  log "[${lane}] repeat ${repeat_id} finished"
}

log "output_root=${OUTPUT_ROOT}"
log "proxy_state=disabled"
log "lane_a_gpus=${LANE_A_GPUS:-0,1,2,3} repeat=${LANE_A_REPEAT_ID:-4}"
log "lane_b_gpus=${LANE_B_GPUS:-4,5,6,7} repeat=${LANE_B_REPEAT_ID:-5}"

run_lane lane_a "${LANE_A_REPEAT_ID:-4}" "${LANE_A_GPUS:-0,1,2,3}" &
pid_a=$!
run_lane lane_b "${LANE_B_REPEAT_ID:-5}" "${LANE_B_GPUS:-4,5,6,7}" &
pid_b=$!
...
log "all rescale=0.5/0.7 repeat lanes finished"
