...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/broad_base_no_tool_no_system_fast_rescale_sweep_20260519}"
...
if [[ ! -s "$STATUS_TSV" ]]; then
  printf "run_name\tscale_id\tinitial_rescale\tstatus\texit_code\tpath\n" > "$STATUS_TSV"
fi
...
verify_fast_path_or_kill() {
  ...
  if kill -0 "$pid" 2>/dev/null; then
    log "ERROR: timed out waiting for fast-path config in $launch_log; killing pid=$pid"
    kill "$pid" 2>/dev/null || true
  fi
  return 1
}
...
run_eval_one() {
  ...
  local scale_value="$2"
  local agent_cfg="$3"
  local run_name="base_no_tool_no_system_fast_hf_val_only_broad_eval_0502_256k_zoom2_area3500_rescale${scale_id}"
  ...
  log "starting $run_name initial_rescale=$scale_value GPUs=$GPUS"
  mkdir -p "$work_dir"
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\n" "$run_name" "$scale_id" "$scale_value" "$status" "$exit_code" "$work_dir" >> "$STATUS_TSV"
...
run_eval_one 07 0.7 recipe/vsearch/config/agent_insight_qwen_agent_zoom_factor2_area3500_rescale07.yaml

log "broad base no-tool/no-system fast rescale sweep finished"
