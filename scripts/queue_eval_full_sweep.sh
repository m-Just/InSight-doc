...
PYTHONPATH_ENTRIES="$REPO_ROOT:$VERL_ROOT"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/workspace/eval_full_sweep_${RUN_ID}}"
AGENT_CONFIG="${AGENT_CONFIG:-evals/agent_configs/insight_qwen_agent_core_zoom_factor2_area3500_rescale025.yaml}"
...
DEFAULT_MODEL_CONFIGS=(
  "evals/model_configs/release_ray_vllm.yaml"
)
...
import sys
from evals.config.model import load_model_config
...
import sys
from evals.config.model import sha256_file
...
if [[ "${#VAL_FILE_ARRAY[@]}" == "0" ]]; then
  ...
  echo "Example: VAL_FILES=/path/a.parquet,/path/b.parquet MODEL_CONFIGS=evals/model_configs/release_ray_vllm.yaml scripts/queue_eval_full_sweep.sh" >&2
  exit 1
fi
