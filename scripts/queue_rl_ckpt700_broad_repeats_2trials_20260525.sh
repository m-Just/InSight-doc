...
MODEL_LABEL="${MODEL_LABEL:-rl_ckpt700_actor_merged_hf}"
LOAD_FORMAT="${LOAD_FORMAT:-auto}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/rl_ckpt700_broad_fast_rescale025_035_05_repeats_20260525}"
...
wait_for_gpu_idle() {
  ...
  local gpus="$2"
  local idle_for=0
  log "${trial}: waiting for GPUs ${gpus} to have no compute processes for ${IDLE_SECONDS}s"
  while (( idle_for < IDLE_SECONDS )); do
    if gpu_has_processes "$gpus"; then
      idle_for=0
      log "${trial}: GPUs ${gpus} busy; polling again in ${POLL_SECONDS}s"
    else
      idle_for=$((idle_for + POLL_SECONDS))
      log "${trial}: GPUs ${gpus} idle for ${idle_for}/${IDLE_SECONDS}s"
    fi
    sleep "$POLL_SECONDS"
  done
}
...
run_eval_one() {
  ...
  local run_name="${MODEL_LABEL}_${trial}_broad_fast_rescale${scale_id}"
  local work_dir="$OUTPUT_ROOT/$run_name"
  local ray_tmp="/tmp/rl700_${trial}_${scale_id}_20260525"
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" \
    "$(date -Is)" "$trial" "$MODEL_LABEL" "$scale_id" "$scale_value" "$status" "$exit_code" "$work_dir" >> "$STATUS_TSV"
...
run_trial() {
  ...
  wait_for_gpu_idle "$trial" "$gpus"
  for scale_id in 025 035 05; do
    run_eval_one "$trial" "$gpus" "$scale_id" || trial_status=1
  done
  log "${trial}: lane finished status=${trial_status}"
  return "$trial_status"
}
...
status=0
run_trial "trialA" "$TRIAL_A_GPUS" &
pid_a=$!
run_trial "trialB" "$TRIAL_B_GPUS" &
pid_b=$!
