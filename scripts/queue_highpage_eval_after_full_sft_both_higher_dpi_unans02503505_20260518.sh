...
SFT_ROOT="${SFT_ROOT:-/scratch/ywxzml3j/likaican/temp/insight_qwen_agent_full_sft_both_higher_dpi_unans02503505_64k_sp2_bs32_8gpu_20260518/full_sft_both_higher_dpi_unans02503505_len65536_sp2_bs32_freeze_vt_epoch1/sft_checkpoints}"
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_full_sft_both_higher_dpi_unans02503505_64k_sp2_bs32_8gpu_20260518}"
MODEL_LABEL="${MODEL_LABEL:-full_sft_both_higher_dpi_unans02503505_len65536_sp2_bs32_epoch1}"
...
wait_for_hf_model() {
  local model_path=""
  while true; do
    if model_path="$(latest_hf_model "${SFT_ROOT}")"; then
      printf '%s\n' "${model_path}"
      return 0
    fi
    log "waiting for HF checkpoint under ${SFT_ROOT}; next check in ${POLL_SECONDS}s"
    sleep "${POLL_SECONDS}"
  done
}
...
main() {
  ...
  log "using HF checkpoint ${model_path}"
