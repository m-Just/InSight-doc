#!/usr/bin/env python3
"""Plot highpage epoch-3 LoRA runs against matched base runs."""
...
RUNS = [
    ...
    (
        "lora_both_bs32_e3",
        "0.25",
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_lora_epoch2_two_lanes_20260515/lora_both_bs32_latest_step1377_highpage_0507_rescale025"),
    ),
    (
        "lora_both_bs32_e3",
        "0.35",
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_lora_epoch2_two_lanes_20260515/lora_both_bs32_latest_step1377_highpage_0507_rescale035"),
    ),
    (
        "lora_both_bs32_e3",
        "0.5",
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_lora_epoch2_two_lanes_20260515/lora_both_bs32_latest_step1377_highpage_0507_rescale05"),
    ),
]

REPLICATE_RUNS = {
    ("base_no_tool_no_system_fast", "0.25"): [
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_base_no_tool_fast_hf_val_only_20260515_fgtest/base_no_tool_no_system_fast_hf_val_only_highpage_0507_rescale025"),
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_base_no_tool_fast_hf_val_only_20260515_remaining/base_no_tool_no_system_fast_hf_val_only_highpage_0507_rescale025"),
    ],
    ("base_no_tool_no_system_fast", "0.35"): [
    ...
    ("base_no_tool_no_system_fast", "0.5"): [
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_base_no_tool_fast_hf_val_only_20260515_fgtest/base_no_tool_no_system_fast_hf_val_only_highpage_0507_rescale05"),
        Path("/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_base_no_tool_fast_hf_val_only_20260515_remaining/base_no_tool_no_system_fast_hf_val_only_highpage_0507_rescale05"),
    ],
}
...
DISPLAY = {
    ...
    "lora_basic_40k_e3": "LoRA basic 40k e3",
    "lora_arxiv_higher_dpi_e3": "LoRA arxiv e3",
    "lora_O3_higher_dpi_e3": "LoRA O3 e3",
    "lora_both_bs16_e3": "LoRA both bs16 e3",
    "lora_both_bs32_e3": "LoRA both bs32 e3",
}
...
COLORS = {
    ...
    "lora_basic_40k_e3": "#2ca02c",
    "lora_arxiv_higher_dpi_e3": "#1f77b4",
    "lora_O3_higher_dpi_e3": "#ff7f0e",
    "lora_both_bs16_e3": "#111111",
    "lora_both_bs32_e3": "#8c564b",
}
...
def mean_std(values: list[float | None]) -> tuple[float | None, float | None]:
    ...
    return sum(clean) / len(clean), std


def aggregate_rows(
    model: str, scale: str, run_dirs: list[Path]
) -> tuple[dict[str, object] | None, list[tuple[str, str, str]]]:
    rows = []
    missing = []
    ...
    aggregate: dict[str, object] = {
        ...
        "scale": scale,
        "n_runs": len(rows),
        "run_dir": ";".join(str(row["run_dir"]) for row in rows),
    }
    for key in [
        "avg_acc",
        "longdoc_acc",
...
def write_csv(rows: list[dict[str, object]], csv_path: Path) -> None:
    fieldnames = [
        ...
        "core_time_s_std",
        "assistant_resp_tokens",
        "assistant_resp_tokens_std",
        "tool_resp_tokens",
        "tool_resp_tokens_std",
        "turns",
        "turns_std",
        "run_dir",
    ]
...
def plot(rows: list[dict[str, object]], out_prefix: Path) -> None:
    ...
    for model in ORDER:
        ...
        xerr = [
            float(row["core_time_s_std"])
            if row.get("core_time_s_std") not in (None, "")
            else 0.0
            for row in points
        ]
        yerr = [
            float(row["avg_acc_std"]) if row.get("avg_acc_std") not in (None, "") else 0.0
            for row in points
        ]
        if any(value > 0 for value in xerr) or any(value > 0 for value in yerr):
            ax.errorbar(
                x,
                y,
                xerr=xerr,
    ...
    ax.set_title("Highpage eval: epoch-3 LoRA models vs base models", fontsize=16)
...
def main() -> None:
    ...
    parser.add_argument("--output-prefix", default="highpage_lora_epoch3_vs_base_2026-05-16")
    ...
    for model, scale, run_dir in RUNS:
        replicate_dirs = REPLICATE_RUNS.get((model, scale))
        if replicate_dirs:
            row, replicate_missing = aggregate_rows(model, scale, replicate_dirs)
            missing.extend(replicate_missing)
        else:
            row = read_row(model, scale, run_dir)
            if row is not None:
                row["n_runs"] = 1
