...
PY
  ...
  EXTRA_SFT_ARGS="optim.lr_scheduler_type=cosine optim.lr_warmup_steps_ratio=0.05 optim.min_lr_ratio=${min_lr_ratio} engine.ulysses_sequence_parallel_size=2" \
    bash "${LAUNCH_SCRIPT}"
  status=$?
  set -e
  end_ts="$(date +%s)"
...
main() {
  ...
  if [ "${USE_VSEARCHER_SYSTEM_FOR_EASY:-0}" == "1" ]; then
    ...
    #   [[ "${CONTINUE_ON_FAILURE}" == "1" ]] || exit 1
    # }
    run_one "lr5e-6_cosine_minlr5e-7_len32768_bs32_full_clean_data_freeze_vt_tool_arg_order_medium_only+p4_5_sp2" "5e-6" "5e-7" "32" || {
      failures=$(( failures + 1 ))
      [[ "${CONTINUE_ON_FAILURE}" == "1" ]] || exit 1
    }
