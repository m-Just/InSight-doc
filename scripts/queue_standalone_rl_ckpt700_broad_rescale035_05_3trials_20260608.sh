...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/workspace/rl_ckpt700_broad_standalone_rescale035_05_3trials_fixed_override_${RUN_ID}}"
...
run_lane() {
  ...
  CUDA_VISIBLE_DEVICES="$gpus" "$PYTHON_BIN" -u scripts/serve_ray_vllm.py \
    ...
    --ray-namespace "standalone_rl700_035_05_${RUN_ID}_${lane_name}" \
...
echo "all standalone rl_ckpt700 broad rescale 0.35/0.5 reruns complete: $OUTPUT_ROOT"
