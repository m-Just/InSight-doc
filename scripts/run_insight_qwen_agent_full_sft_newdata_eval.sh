...
export SFT_WORK_DIR="${SFT_WORK_DIR:-/scratch/ywxzml3j/likaican/temp/insight_qwen_agent_full_sft_newdata}"
export WORK_DIR="${WORK_DIR:-$OUTPUT_ROOT/insight_qwen_agent_full_sft_newdata_eval}"
export PROJECT_NAME="${PROJECT_NAME:-insight_doc}"
export EXP_NAME="${EXP_NAME:-insight_qwen_agent_full_sft_newdata_eval}"
...
export EVAL_MAX_MODEL_LEN="${EVAL_MAX_MODEL_LEN:-32768}"

if [[ -z "${MODEL_PATH:-}" ]]; then
  MODEL_PATH="$(SFT_WORK_DIR="${SFT_WORK_DIR}" python - <<'PY'
from pathlib import Path
import os

work_dir = Path(os.environ["SFT_WORK_DIR"])
...
for step_dir in ckpt_root.glob("global_step_*"):
    ...
    if hf_dir.is_dir() and (best_step is None or step > best_step):
        best = hf_dir
        best_step = step
if best is None:
    raise SystemExit(f"No huggingface checkpoint found under {ckpt_root}")
print(best)
PY
)"
fi
...
run_experiment \
  ...
  +actor_rollout_ref.rollout.engine_kwargs.vllm.max_model_len="$EVAL_MAX_MODEL_LEN"
