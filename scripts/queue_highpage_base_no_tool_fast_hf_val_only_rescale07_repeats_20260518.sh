...
# Keep this eval path direct; earlier proxy use introduced avoidable failures.
...
OUTPUT_ROOT="${OUTPUT_ROOT:-/home/ywxzml3j/ywxzml3juser40/insight_doc/outputs/highpage_base_no_tool_fast_hf_val_only_rescale07_repeats_20260518}"
...
if [[ "${RESET_STATUS:-0}" == "1" || ! -s "$STATUS_TSV" ]]; then
  printf "repeat_id\tmodel\tscale_id\tinitial_rescale\tstatus\texit_code\tpath\n" > "$STATUS_TSV"
fi
...
run_eval_one() {
  ...
  log "starting ${run_name} gpus=${GPUS} initial_rescale=${scale_value}"
  mkdir -p "$work_dir"
  ...
  printf "%s\t%s\t%s\t%s\t%s\t%s\t%s\n" "$repeat_id" "$model_label" "$scale_id" "$scale_value" "$status" "$exit_code" "$work_dir" >> "$STATUS_TSV"
...
wait_for_idle_gpus
run_eval_one 3
log "all rescale=0.7 repeats finished"
