...
INPUT_ROOT="${INPUT_ROOT:-${VERL_ROOT}/artifacts/synthetic_unanswerable_pipeline/by_rescale_20260518/easy}"
RUN_TAG="${RUN_TAG:-retry_easy_by_rescale_20260518}"
EXP_NAME_BASE="${EXP_NAME_BASE:-synthetic_unanswerable_qwen3_region_loc_easy_retry_by_rescale}"
...
run_one() {
  ...
  echo "==> synthetic unanswerable easy rescale${rescale}"
...
run_one "025" \
  "${INPUT_ROOT}/rescale025/sft_data.vreasoner_v2_easy_rescale025.parquet" \
  "recipe/vsearch/config/agent_gpt-5-mini_vr2_zoom_factor2_initial_0_25_max_calls10_unanswerable_aware_verify_explicit_evidence_linked.yaml"

run_one "035" \
  "${INPUT_ROOT}/rescale035/sft_data.vreasoner_v2_easy_rescale035.parquet" \
  "recipe/vsearch/config/agent_gpt-5-mini_vr2_zoom_factor2_initial_0_35_max_calls10_unanswerable_aware_verify_explicit_evidence_linked.yaml"

run_one "05" \
  "${INPUT_ROOT}/rescale05/sft_data.vreasoner_v2_easy_rescale05.parquet" \
  "recipe/vsearch/config/agent_gpt-5-mini_vr2_zoom_factor2_initial_0_5_max_calls10_unanswerable_aware_verify_explicit_evidence_linked.yaml"

echo "Done: synthetic unanswerable easy rescale 0.25 / 0.35 / 0.5"
