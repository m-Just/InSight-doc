...
if [[ "${TELEGRAM_NOTIFY_ON_FINISH:-1}" == "1" && "${TELEGRAM_WRAPPED:-0}" != "1" && -x "$SCRIPT_DIR/run_with_telegram_notification.sh" ]]; then
  ...
  exec "$SCRIPT_DIR/run_with_telegram_notification.sh" \
    --label "${TELEGRAM_NOTIFY_LABEL:-standalone_single_model_broad_eval}" \
    -- "$0" "$@"
fi

cd "$REPO_ROOT"

: "${MODEL_CONFIG:?MODEL_CONFIG must point to one standalone_eval model_config YAML}"
: "${OPENAI_API_KEY:?OPENAI_API_KEY must be set for HTTPS generation and/or judging}"
: "${OPENAI_BASE_URL:?OPENAI_BASE_URL must be set for HTTPS generation and/or judging}"
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$REPO_ROOT/workspace/single_model_broad_standalone_${RUN_ID}}"
...
for line in rollout_path.open():
    ...
    obj = json.loads(line)
    sample = obj.get("sample", obj)
    rollout_noncritical += int(not sample.get("critical_failure"))
scored = 0
if scores_path.exists():
    for line in scores_path.open():
        ...
        obj = json.loads(line)
        sample = obj.get("sample", obj)
        scored += int(bool(sample.get("score")))
...
run_rollout_with_retries() {
  ...
  local rollout_extra_args=()
  if [[ -n "$HTTPS_TIMEOUT_OVERRIDE" ]]; then
    rollout_extra_args+=(--https-timeout-override "$HTTPS_TIMEOUT_OVERRIDE")
  fi
  if [[ -n "$HTTPS_MAX_RETRIES_OVERRIDE" ]]; then
    rollout_extra_args+=(--https-max-retries-override "$HTTPS_MAX_RETRIES_OVERRIDE")
  fi
...
run_one() {
...

  {
    ...
    echo "model=$model"
    echo "model_config=$(realpath "$MODEL_CONFIG")"
    ...
    echo "https_timeout_override=${HTTPS_TIMEOUT_OVERRIDE:-}"
    echo "https_max_retries_override=${HTTPS_MAX_RETRIES_OVERRIDE:-}"
  } > "$output_dir/logs/run.env"

  run_rollout_with_retries "$model" "$rescale" "$output_dir"
  run_judge_with_retries "$model" "$rescale" "$output_dir"
  echo "[$(date -u +'%F %T')] done model=$model rescale=$rescale output=$output_dir"
}

MODEL_NAME="${MODEL_NAME:-$(read_model_name)}"
export API_LOGGER_PROJECT_NAME="${API_LOGGER_PROJECT_NAME:-standalone_${MODEL_NAME//[^A-Za-z0-9_]/_}_broad_eval}"
