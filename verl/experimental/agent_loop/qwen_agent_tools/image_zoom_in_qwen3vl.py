...
"""Local copy of Qwen-Agent's Qwen3-VL image zoom tool.

This registers itself into qwen_agent's TOOL_REGISTRY under
``image_zoom_in_tool_qwen3vl`` so callers can opt into the Qwen3-VL-specific
behavior without changing the upstream meaning of ``image_zoom_in_tool``.
"""
...
@register_tool("image_zoom_in_tool_qwen3vl")
    ...
    def call(self, params: Union[str, dict], **kwargs) -> List[ContentItem]:
        ...
        try:
            ...
            if image_arg.startswith("data:image"):
                image_arg = image_arg.split("base64,", 1)[1]
                image = Image.open(BytesIO(base64.b64decode(image_arg)))
