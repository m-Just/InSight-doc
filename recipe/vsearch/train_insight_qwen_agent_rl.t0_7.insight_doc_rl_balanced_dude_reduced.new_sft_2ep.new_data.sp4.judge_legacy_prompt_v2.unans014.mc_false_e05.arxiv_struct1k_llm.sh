...
export EXP_NAME="insight_doc_rl_balanced_dude_reduced_u25_qwen3_insight_qwen_agent_rl_t0_7_def_sparams_new_sft_2ep_new_data_sp4_rl16k_prompt24k_weighted_refill_simple_source_unans014_mc_false_e05_arxiv_struct1k_llm_judge_legacy_prompt_v2"
...
# This variant preserves total unanswerable sampling mass at 14%, keeps 5%
# mass for filtered MC false-E rows, and assigns 5% total mass to the
# LLM-rewritten arXiv structural LongDocURL-like rows. The structural 5% is
# stolen only from answerable arXiv rows: 2.8% from arxiv_veqa_answerable for
# single-answer structural QAs, and 2.2% from arxiv_mveqa_answerable for
# list-answer structural QAs. The VSearchWeightedRandomRefillBatchSampler
# samples sources by these probabilities and refills each source-specific row
# pool after it exhausts.
#
# Judge variant: legacy_prompt_v2 keeps the legacy two-step extraction/verifier
# structure but uses the minimal prompt fixes for answer truncation, refusal
# preservation, final MC option selection, and answerable-refusal rejection.
run_experiment \
    +custom_reward_function.reward_kwargs.insight_qwen_judge_mode=legacy_prompt_v2 \
