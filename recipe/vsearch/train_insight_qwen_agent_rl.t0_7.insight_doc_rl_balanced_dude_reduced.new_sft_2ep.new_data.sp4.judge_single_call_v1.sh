...
export EXP_NAME="insight_doc_rl_balanced_dude_reduced_u25_qwen3_insight_qwen_agent_rl_t0_7_def_sparams_new_sft_2ep_new_data_sp4_rl16k_prompt24k_weighted_refill_simple_source_judge_single_call_v1"
...
export TRAIN_FILES='[/scratch/ywxzml3j/likaican/src/verl-qwen3-vl/notes/generated/rl_16k_prompt24k_r05_to_r035_simple_datasource_20260617/insight_doc_rl_16k_prompt24k_r05_to_r035_simple_datasource-insight_qwen_agent.parquet]'
...
# recipe/vsearch/config/insight_doc_rl_category_answerability_sampling_weights_prompt24k_simple_source_custom_20260617.yaml.
# The train parquet uses simplified data_source keys: {category}_{answerability};
# original/current rescale is stored in extra_info and is not part of the sampler key.
# Category marginals:
#   arxiv_mveqa:0.185, arxiv_veqa:0.185, docvqa:0.100, dude:0.225,
#   info:0.040, map_metro:0.150, map_travel:0.040, poster:0.075.
# Answerable/unanswerable splits:
#   info=0.70/0.30, map_metro=0.80/0.20, poster=0.60/0.40,
#   all other categories=0.85/0.15.
#
# category            answerable       unanswerable               total
# ━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━  ━━━━━━━━━━━━━━━━━━
# arxiv_mveqa    15.725% (1.11x)     2.775% (0.25x)     18.500% (0.74x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# arxiv_veqa     15.725% (1.08x)     2.775% (0.21x)     18.500% (0.66x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# docvqa          8.500% (3.82x)     1.500% (0.44x)     10.000% (1.77x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# dude           19.125% (2.79x)     3.375% (0.19x)     22.500% (0.90x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# info            2.800% (3.72x)     1.200% (1.11x)      4.000% (2.18x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# map_metro      12.000% (3.84x)     3.000% (1.91x)     15.000% (3.20x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# map_travel      3.400% (1.08x)     0.600% (0.69x)      4.000% (0.99x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# poster          4.500% (3.65x)     3.000% (0.68x)      7.500% (1.34x)
# ─────────────  ─────────────────  ─────────────────  ──────────────────
# total          81.775% (1.77x)    18.225% (0.34x)    100.000% (1.00x)
#
# The VSearchWeightedRandomRefillBatchSampler samples sources by these probabilities
# and refills each source-specific row pool after it exhausts.
run_experiment \
    ...
    data.batch_sampler.weights_file=/scratch/ywxzml3j/likaican/src/verl-qwen3-vl/recipe/vsearch/config/insight_doc_rl_category_answerability_sampling_weights_prompt24k_simple_source_custom_20260617.yaml \
